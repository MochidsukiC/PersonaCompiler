...
it('exposes organization creation through the real App Server with a local Responses provider', async () => {
  await mkdir('.local/tests', { recursive: true })
  const root = await mkdtemp(path.resolve('.local/tests/organization-tools-'))
  ...
  const provider = http.createServer((request, response) => {
    ...
    request.on('end', () => {
      ...
      const marriage = last?.role === 'user' && JSON.stringify(last).includes('ORGANIZATION_FIXTURE')
      const item = marriage
        ? { id: crypto.randomUUID(), type: 'function_call', call_id: crypto.randomUUID(), name: 'createOrganization', arguments: JSON.stringify({ name: '共同工房', type: '会社', purpose: '道具を作る', locationId: null }) }
  ...
  runtime.setToolHandler(async call => { calls.push(call); lifeToolSchemas.createOrganization.parse(call.arguments); return { success: true, contentItems: [{ type: 'inputText', text: '{"status":"proposed"}' }] } })
  try {
    ...
    expect(tools.map(t => t.name)).toEqual(expect.arrayContaining(['createOrganization', 'joinOrganization', 'leaveOrganization']))
    ...
    const turn = await runtime.startTurn(binding, 'ORGANIZATION_FIXTURE')
    await expect.poll(() => completed.some(t => t.includes(turn)), { timeout: 20000 }).toBe(true)
    expect(calls).toHaveLength(1)
    expect(calls[0]).toMatchObject({ threadId: binding.threadId, tool: 'createOrganization', arguments: { name: '共同工房', type: '会社', purpose: '道具を作る', locationId: null } })
