...
import { lifeTools, lifeToolSchemas } from '../../src/core/life-contracts'
import type { SessionBinding } from '../../src/core/contracts'

it('exposes facility construction through the real App Server with a local Responses provider', async () => {
  await mkdir('.local/tests', { recursive: true })
  const root = await mkdtemp(path.resolve('.local/tests/construction-tools-'))
  ...
  const provider = http.createServer((request, response) => {
    ...
    request.on('end', () => {
      ...
      const marriage = last?.role === 'user' && JSON.stringify(last).includes('BUILD_FIXTURE')
      const item = marriage
        ? { id: crypto.randomUUID(), type: 'function_call', call_id: crypto.randomUUID(), name: 'buildFacility', arguments: JSON.stringify({ name: '共同工房', type: 'workshop', description: '道具を作る', dimensions: { x: 20, y: 20, z: 3 }, organizationId: null }) }
  ...
  runtime.setToolHandler(async call => { calls.push(call); lifeToolSchemas.buildFacility.parse(call.arguments); return { success: true, contentItems: [{ type: 'inputText', text: '{"status":"proposed"}' }] } })
  try {
    ...
    expect(tools.map(t => t.name)).toEqual(expect.arrayContaining(['buildFacility', 'joinOrganization', 'leaveOrganization']))
    ...
    const turn = await runtime.startTurn(binding, 'BUILD_FIXTURE')
    await expect.poll(() => completed.some(t => t.includes(turn)), { timeout: 20000 }).toBe(true)
    expect(calls).toHaveLength(1)
    expect(calls[0]).toMatchObject({ threadId: binding.threadId, tool: 'buildFacility', arguments: { name: '共同工房', type: 'workshop', description: '道具を作る', dimensions: { x: 20, y: 20, z: 3 }, organizationId: null } })
