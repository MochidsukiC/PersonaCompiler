...
test('searches saved events, pages at a fixed revision, reports errors and ignores late results after returning live', async () => {
  ...
  try {
    ...
    const copyReport = timeline.getByRole('button', { name: 'QAレポートをコピー' })
    const clipboardText = () => app.evaluate(({ clipboard }) => clipboard.readText())
    await copyReport.click()
    await expect.poll(clipboardText).toContain('対象: 画面に届いた直近の出来事')
    expect(await clipboardText()).toContain(`ワールドID: ${original.state.runId}`)
    expect(await clipboardText()).toContain('## #240 · Turn 0 · 発話')
    ...
    await copyReport.click()
    await expect.poll(clipboardText).toContain('このページ: 101–200 / 検索一致 240件')
    const report = await clipboardText()
    await writeFile(test.info().outputPath('event-qa-report.md'), report)
    expect(report).toContain('保存revision 3')
    expect(report).toContain('## #140 · Turn 0 · 発話')
    expect(report).not.toContain('## #240 · Turn 0 · 発話')
    expect([...report.matchAll(/^## #\d+/gm)]).toHaveLength(100)
    ...
    await copyReport.click()
    await expect.poll(clipboardText).toContain('"query": "約束"')
    expect(await clipboardText()).toContain('## #1 · Turn 0 · 発話')
    ...
    await copyReport.click()
    await expect(timeline.getByRole('alert')).toContainText('レポートをコピーできません: fixture: clipboard unavailable')
    await page.evaluate(() => Reflect.deleteProperty(navigator.clipboard, 'writeText'))
    await copyReport.click()
    await expect(timeline.getByText('表示中の1件と検索条件をコピーしました。')).toBeVisible()
