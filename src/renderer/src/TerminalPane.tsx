...
export function TerminalPane({ agent, real = false, connected = true, memoryEnabled = false, readOnly = false, onError }: { agent: AgentDescriptor; real?: boolean; connected?: boolean; memoryEnabled?: boolean; readOnly?: boolean; onError: (message: string) => void }) {
  ...
  useLayoutEffect(() => {
    setReady(false)
    if (!container.current || !connected || readOnly) return
  ...
  }, [agent.sessionId, connected, readOnly, generation, onError])
  ...
  return <section className="terminal-panel" aria-label={`${agent.name}の端末`}>
    ...
    <div className="terminal-person"><span className="avatar" style={{ '--agent-color': agent.color } as React.CSSProperties}>{agent.name.slice(-1)}</span><div><strong>{agent.name}</strong><small>{roleLabels[agent.role]} <span>• {readOnly ? '履歴・閲覧専用' : !connected ? 'Codexへの接続待ち' : agent.status === 'ended' ? '端末終了・会話は保存済み' : agent.status === 'interrupted' ? '中断中' : agent.status === 'idle' ? '入力待ち' : '接続中'}</span></small></div>{real && <button className="button compact" disabled={readOnly || !connected || reconnecting || agent.status !== 'ended'} onClick={() => void reconnect()}>{reconnecting ? '再接続中…' : '再接続'}</button>}<button className="icon-button interrupt" disabled={readOnly || !connected || agent.status === 'ended'} title="処理に割り込む（Sessionは保持）" aria-label="処理に割り込む" onClick={() => void window.persona.terminalInterrupt(agent.sessionId).catch(onError)}><Square size={13} /></button></div>
