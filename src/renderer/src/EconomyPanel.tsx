...
export function EconomyPanel({ world, onAgent }: { world: SimulationSnapshot; onAgent: (id: string) => void }) {
  ...
  return <section className="economy-panel" aria-label="経済"><header><h1>経済・アイテム</h1><p>住民の仕事と取引、品物の動きを観測します。</p></header>
    ...
    <h2>カタログ</h2><div className="economy-grid">{economy.catalog.map(({ item, licensees }) => <article key={item.id}><h3>{item.name}</h3><p>{item.description}</p><p>{item.primary ? `一次産品 · 1勤務 ${item.primary.yield}個 · 出荷単価 ${item.primary.exportPrice}${economy.currency}` : `生成費用 ${item.cost}${economy.currency}`}</p><p>HP {item.effects.hp >= 0 ? '+' : ''}{item.effects.hp} / 空腹 {item.effects.hunger >= 0 ? '+' : ''}{item.effects.hunger} / SAN {item.effects.san >= 0 ? '+' : ''}{item.effects.san}{item.durability !== null && ` / 耐久 ${item.durability}`}</p><small>取得権: {licensees.map(o => ownerName(world, o)).join('、')}</small></article>)}</div>
