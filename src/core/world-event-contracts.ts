...
const payload = z.object({ title: z.string().trim().min(1).max(160), type: z.string().trim().min(1).max(80), description: z.string().trim().min(1).max(4000), target: worldEventTargetSchema }).strict()
...
export function worldEventTools() {
  return Object.entries(worldEventToolSchemas).map(([name, schema]) => ({ type: 'function' as const, name, description: descriptions[name as keyof typeof descriptions], inputSchema: z.toJSONSchema(schema) }))
}
