...
export class ParentProduction {
  ...
  private async execute(kind: ProductionOperation['kind'], targetId: string, prompt: string, input: unknown): Promise<unknown> {
    ...
    await this.workspace.write(`${directory}/input.json`, JSON.stringify(input, null, 2))
    ...
    try {
      ...
      operation.status = completionKnown ? 'failed' : 'uncertain'; operation.error = error instanceof Error ? error.message : String(error)
      await this.changed(operation)
      throw error
    } finally { off() }
  }
