...
生活の推論が一時的なrate limitで失敗した場合は、最大5回まで自動再試行します。待機は1・2・4・8・16秒を基準に、エラーの指定時間以上とし、小さなランダム待機を加えます。待機中は生活スケジューラーからの新規推論を保留します。同じConversationへ未完了部分の継続を要求し、元の依頼文は再送しません。CompactはCompact処理を再試行します。待機期限・試行回数・元のエラーは保存対象で、一時停止・終了中には再試行せず、再開時に残りの待機時間を守ります。指定待機が60秒を超える場合、待機指定を解釈できない場合、試行回数の上限、課金・quotaエラー、結果不明の送信はエラー停止します。親の準備・制作推論と記憶照合はこの再試行の対象外です。
