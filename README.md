...
`moveWithinFacility` は無制限の即時移動、`moveToFacility` は次ターンの移動予約です。予約はターン内に1回で、発話によって活動を再開しても上書きできません。入場位置が全員分揃ってから生活を開始します。

声量はlowが直線距離1、mediumが5、highが同施設全体。家内の声は同じ家にだけ届き、屋外の声は家内にも届きます。睡眠中のNPCへは発話を届けず、起床後にも再配信しません。状況取得は同施設内の全NPCの公開座標を返します。
...
- 単体テスト: 既存デモ・ファイル監視・端末連番、Auto、年齢境界、人口配分、質問・承認、準備の復旧に加え、家の検証、3次元距離、家の片方向の遮音、移動予約、活動再開、睡眠、配信重複防止、日付更新を確認します。
