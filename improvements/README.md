...
| `git log --oneline -- improvements/14-terminal-startup-shutdown.md`で確認 | 起動が遅い端末の停止時に未確定PIDへシグナルを送る不具合を修正 | [14](14-terminal-startup-shutdown.md) |
...
最終ソースでbuild・typecheck・lint成功、単体46・backend129・CLI接続と端末回帰8・保存4・Electron E2E18の計205件PASS。最新の終了コードとログは[14](14-terminal-startup-shutdown.md)に記載しています。端末の同時終了時のnative競合には[12](12-native-terminal-exit.md)で上流修正版を適用し、[13](13-terminal-resource-cleanup.md)でworker残留を修正してクリーンインストールも検証しました。さらに起動が遅い端末の停止時に未確定PIDへシグナルを送る不具合を修正しました。過去の別の終了コード`3221226505`の直接のstackは未取得であり、過去の全異常終了を解消したとは断定しません。既存のテスト除外・閾値・警告設定は変更していません。
