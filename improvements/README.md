...
| `git log --oneline -- improvements/13-terminal-resource-cleanup.md`で確認 | 端末の終了・再接続で出力workerが残る不具合を修正 | [13](13-terminal-resource-cleanup.md) |
...
クリーンインストール後の最終ソースでbuild・typecheck・lint成功、単体46・backend125・CLI接続と端末回帰8・保存4・Electron E2E18の計201件PASS。最新の終了コードとログは[13](13-terminal-resource-cleanup.md)に記載しています。端末の同時終了時のnative競合には[12](12-native-terminal-exit.md)で上流修正版を適用しました。その後、再接続のたびに出力workerが残る別の不具合を確認して修正し、最小スクリプトのホストNodeも自然終了するようになりました。過去の別の終了コード`3221226505`の直接のstackは未取得であり、過去の全異常終了を解消したとは断定しません。既存のテスト除外・閾値・警告設定は変更していません。
