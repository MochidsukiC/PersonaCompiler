...
2026-09-12の追加仕様として、新規ワールドは以下を採用する。以降の出生・記憶・観測・Compilationに関する章は将来段階の設計であり、今回の実装範囲には含めない。
...
- `sleep`は1ターンの睡眠。毎回同じConversationでNative Compactし、完了確認後に次ターンへ進む。独自の記憶整理・想起は次段階。
