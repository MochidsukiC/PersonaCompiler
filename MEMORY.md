...
- `sleep`後は現在の推論を終え、本人のConversationで整理します。候補・保持記憶・関係に加え、本人の直近最大30件の経験を`memorySources`として渡します。未登録の経験は本人が`remember`で選び、返された`candidateId`を使って`consolidateMemory`で記憶と本人から相手への認識を一括確定します。確定後の追加登録はできません。推論終了を確認してからNative Compactを開始します。
...
経過日数による関係の自動生成はありません。本人が記憶を選び、その記憶を根拠に関係を整理した後に矢印が現れます。0件のときは画面にこの表示条件を示します。
